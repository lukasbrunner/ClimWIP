...
"""
Time-stamp: <2020-09-07 16:46:30 lukas>
...
def boxplot(ax,
    ...
    """
    if data is not None:
        if mean is None and showmean:
            mean = data
        if median is None and showmedian:
            median = data
        if box is None and showbox:
            box = data
        if whis is None and showwhis:
            whis = data
        if dots is None and showdots:
            dots = data
    ...
    elif box is not None and tuple(box) == (None, None):
        box = None
    ...
    if 's' not in dots_kwargs.keys():
        ...
        if weights is not None and dots_sizes is not None:
            sizes = np.interp(weights, np.quantile(weights, (.05, .95)), dots_sizes)
