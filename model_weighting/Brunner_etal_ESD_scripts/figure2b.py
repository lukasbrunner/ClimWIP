...
FILENAME = 'tas_global_pseudo_obs_{ssp}_050_MPI-ESM-LR-r1i1p1-CMIP5-{rcp}_81-00_ind.nc'
...
def load_data(*args, filenames=None):
    ...
    if len(filenames) > 1:
        filenames = remove_models(filenames, 'MPI	')
...
def plot_single(ax, cmip5_rcp26, cmip6_ssp126, cmip5_rcp85, cmip6_ssp585,
    ...
    handles.append((hh1, hh2))
    labels.append('CMIP6 SSP585 mean & 66% range weighted')
...
def main():
    ...
    cmip5_rcp26 = load_data('CMIP5', 'rcp26', ['MPI-ESM-LR_r1i1p1_CMIP5'], '/net/atmos/data/cmip5-ng')
    ...
    cmip5_rcp85 = load_data('CMIP5', 'rcp85', ['MPI-ESM-LR_r1i1p1_CMIP5'], '/net/atmos/data/cmip5-ng')
    ...
    weights_ssp126_tasTREND = xr.open_dataset(
        os.path.join(
            LOADPATH,
            'tas_global_pseudo_obs_{ssp}_tasTREND_MPI-ESM-LR-r1i1p1-CMIP5-{rcp}_81-00_ind.nc'.format(
    ...
    fig.subplots_adjust(left=.05, right=.98, bottom=.05, top=.94)
    ...
    ax.set_title('\\textbf{(b) Combined weighting based on pseudo-observations from MPI-ESM-LR (CMIP5)}', loc='left',
                 fontdict={'size': 'large', 'weight': 'bold'})

    plt.savefig(os.path.join(SAVEPATH, 'figure2b.png'), dpi=300)
    plt.savefig(os.path.join(SAVEPATH2, 'figure2b.pdf'), dpi=300)
