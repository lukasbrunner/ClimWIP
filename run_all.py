...
DATA_PATH = '/net/h2o/climphys/lukbrunn/Data/ModelWeighting/EUCP/'
PLOT_PATH = '/net/h2o/climphys/lukbrunn/Plots/ModelWeighting/'
...
for section in config.sections():
    if config[section]['save_path'] is None:
        config[section]['save_path'] = os.path.join(
            DATA_PATH, os.path.basename(args.filename.replace('.ini', '')))
    if config[section]['plot_path'] is None:
        config[section]['plot_path'] = os.path.join(
            PLOT_PATH, os.path.basename(args.filename.replace('.ini', '')), 'process_plots')
